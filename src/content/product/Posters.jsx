...
class Posters extends React.Component {
  static async fetchData(search, showError) {
    ...
    const vars = { sourceSheet: 'Posters' };
  ...
  async loadData() {
    ...
    const data = await Posters.fetchData(search, showError);
    if (data) {
      this.setState({
...
const ArtWithToast = withToast(Posters);
ArtWithToast.fetchData = Posters.fetchData;
