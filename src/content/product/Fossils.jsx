...
class Fossils extends React.Component {
  static async fetchData(search, showError) {
    ...
    const vars = { sourceSheet: 'Fossils' };
  ...
  async loadData() {
    ...
    const data = await Fossils.fetchData(search, showError);
    if (data) {
      this.setState({
...
const FossilsWithToast = withToast(Fossils);
FossilsWithToast.fetchData = Fossils.fetchData;

export default FossilsWithToast;
